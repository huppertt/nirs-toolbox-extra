function [SPM] = nirs_spm_indiv(SPM, Y, save_directory)
% In an individual analysis, GLM parameters and temporal correlations are
% estimated.
% Last update : November 24th, 2009.
tic
xX            = SPM.xX;
...
% update for calculating spatial correlations between different channel
% residuals (v3_3)
...
for aa = 1:nCh
    for bb = 1:nCh
        ...
        elseif aa ~= bb
            ResSS_nirs(aa,bb) = (KWY(:,aa)'*KWY(:,bb) -  KWY(:,aa)'*xX.xKXs.X*xX.pKX* KWY(:,bb));
        end
    end
end
% end of updates 
