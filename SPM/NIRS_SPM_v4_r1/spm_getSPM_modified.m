...
% 2-D case
%-----------------------------------------------------------------------
if  strcmp('FMRI', 'EEG') & SPM.xVol.DIM(3) == 1
...
%-----------------------------------------------------------------------
try
    if strcmp('FMRI', 'EEG') & isfield(SPM.xX, 'fullrank')
        Vbeta = SPM.Vbeta;
    else
...
%-Get contrasts
%-----------------------------------------------------------------------
[Ic,xCon] = spm_conman_modified(SPM,'T&F',Inf,...
		'	Select contrasts...',' for conjunction',1);


nc       = length(Ic);  % Number of contrasts
% num_tmp = 10000 + Ic;
% num_tmp = num2str(num_tmp);
% 
% %%% modified (NIRS-SPM)
% xCon(Ic).Vspm.fname = [swd filesep 'spm' xCon(Ic).STAT '_' num_tmp(2:end) '.img'];
...
if spm_input('mask with other contrast(s)','+1','y/n',[1,0],2)
	[Im,xCon] = spm_conman_modified(SPM,'T&F',-Inf,...
