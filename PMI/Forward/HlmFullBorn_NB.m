...
function [A, Phi_Inc] = HlmFullBorn_NB(pmiModel, k, MeasList, zBnd, Debug)
...
for idx = 1:num2
  ...
  %% Added for non-linear update of Jacobian
  Pos = [Xm(:) Ym(:) Zm(:)];
  for idxDet = 1:nDets
    Det = ones(size(Pos,1),1) * pDet(idxDet,:);
  
    rdetpos = sqrt( sum((Det - Pos).^2,2) );

    Phi_Inc(1:size(rdetpos,1),nSrcs+idxDet) = exp(j * k * rdetpos) ./ (-4 * pi * rdetpos);    
  end
