%Hlm3DScatPntJacZB  Jacobian iteration 3D Helmholtz scattered field solver.
%
...
%  initial
%
%  Revision 3.2  2000/03/10 21:40:21  rjg
%  Fixed a typo in the in the indexing of PhiNew. iY -> iX.
%
%  Revision 3.1  1999/11/15 16:34:44  rjg
%  Optimizations: moved non constants out of the inner loop (dt_EffSrc, CtrCoef).
%
...
for iSrc = 1:nSrc
    ...
    %%
    %%  Just borrowing the variable PhiOld, it is really the incident field
    ...
    %%
    dt_EffSrc = dt * PhiOld(iY, iX, iZ) .* del_ksq(iY, iX, iZ);
    CtrCoef = (1 - 2 * (ry + rx + rz) + dt * ksq(iY,iX,iZ));
    ...
    for nIter = 1:nIterMax
        PhiNew(iY,iX,iZ) =  ...
            ...
            CtrCoef .* PhiOld(iY,iX,iZ) + dt_EffSrc;
