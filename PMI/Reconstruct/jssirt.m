...
%  initial
%
%  Revision 1.0  1998/09/22 17:57:22  rjg
%  Initial revision
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [x, C, xmean] = jssirt(A, b, x, nIter, nBlk, C)
...
for i = 1:nBlk
    %%
    %%  SIRT on the first system
    %%
    x1 = sirt_iter(A(:,:,1), b(:,1), x1, rownorm1, nIter);

    %%
    %%  Estimate of C, and then average the two results to get x2
    %%
    if nargin < 6
        [junk x2 xmean C] = cest(x1, x2);
    else
        [junk x2 xmean] = cest(x1, x2, C);
    end
    
    %%
    %%  SIRT on the second system
    %%
    x2 = sirt_iter(A(:,:,2), b(:,2), x2, rownorm2, nIter);

    %%
    %%  Estimate of C, and then average the two results to get x1
    %%
    if nargin < 6
        [x1 junk xmean C] = cest(x1, x2);
    else
        [x1 junk xmean] = cest(x1, x2, C);
    end
end
