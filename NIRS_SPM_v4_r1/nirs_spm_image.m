function nirs_spm_image(op,varargin)
...
if strcmp(op,'standalone');
   ...
   nirs_spm_image('init', template_fname);   
...
if strcmp(op,'repos'),
    ...
    % spm_orthviews('MaxBB');
    nirs_spm_image('zoom_in');
    nirs_spm_image('update_info');
...
if strcmp(op,'addblobs'),
    ...
    for i=1:nblobs,
        ...
        set(st.blobber,'String','Remove Blobs','Callback','nirs_spm_image(''rmblobs'');');
    end;
...
if strcmp(op,'rmblobs'),
    ...
    set(st.blobber,'String','Add Blobs','Callback','nirs_spm_image(''addblobs'');');
end;
...
if strcmp(op,'reorient'),
    ...
    if sum((tmp(:)-st.vols{1}.mat(:)).^2) > 1e-8,
        nirs_spm_image('init',st.vols{1}.fname);
    end;
    return;
end;
...
if strcmp(op,'init'),
    ...
    spm_orthviews('MaxBB');
    st.callback = 'nirs_spm_image(''shopos'');';
    ...
    %-----------------------------------------------------------------------
    uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','nirs_spm_image(''reset'');');
    ...
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',1)','Position',[175 220 065 020].*WS,'String','0','ToolTipString','translate');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',2)','Position',[175 200 065 020].*WS,'String','0','ToolTipString','translate');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',3)','Position',[175 180 065 020].*WS,'String','0','ToolTipString','translate');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',4)','Position',[175 160 065 020].*WS,'String','0','ToolTipString','rotate');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',5)','Position',[175 140 065 020].*WS,'String','0','ToolTipString','rotate');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',6)','Position',[175 120 065 020].*WS,'String','0','ToolTipString','rotate');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',7)','Position',[175 100 065 020].*WS,'String','1','ToolTipString','zoom');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',8)','Position',[175  80 065 020].*WS,'String','1','ToolTipString','zoom');
    uicontrol(fg,'Style','edit','Callback','nirs_spm_image(''repos'',9)','Position',[175  60 065 020].*WS,'String','1','ToolTipString','zoom');

    uicontrol(fg,'Style','Pushbutton','String','Reorient images...','Callback','nirs_spm_image(''reorient'')',...
        'Position',[70 35 125 020].*WS,'ToolTipString','modify position information of selected images');

    uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','nirs_spm_image(''resetorient'')',...
        'Position',[195 35 55 020].*WS,'ToolTipString','reset orientations of selected images');
    
    uicontrol(fg,'Style','Pushbutton','String','+','FontSize',13,'Callback','nirs_spm_image(''addindiloc'')',...
        'Position',[265 290 35 020].*WS);
    uicontrol(fg,'Style','Pushbutton','String','-','FontSize',15,'Callback','nirs_spm_image(''delindiloc'')',...
    ...
    st.mp = uicontrol(fg,'Style','edit', 'Position',[110 295 135 020].*WS,'String','','Callback','nirs_spm_image(''setposmm'')','ToolTipString','move crosshairs to mm coordinates');
    st.vp = uicontrol(fg,'Style','edit', 'Position',[110 275 135 020].*WS,'String','','Callback','nirs_spm_image(''setposvx'')','ToolTipString','move crosshairs to voxel coordinates');
    ...
    st.zoomer = uicontrol(fg,'Style','popupmenu' ,'Position',[315 75 125 20].*WS,...
        ...
        'Callback','nirs_spm_image(''zoom_in'')','ToolTipString','zoom in by different amounts');
    c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;nirs_spm_image(''zoom_in'')';
    ...
    st.win = uicontrol(fg,'Style','popupmenu','Position',[315 35 125 20].*WS,...
        'String',char('Auto Window','Manual Window'),'Callback','nirs_spm_image(''window'');','ToolTipString','range of voxel intensities displayed');
    % uicontrol(fg,'Style','pushbutton','Position',[315 35 125 20].*WS,...
    % 	'String','Window','Callback','nirs_spm_image(''window'');','ToolTipString','range of voxel intensities % displayed');
    st.blobber = uicontrol(fg,'Style','pushbutton','Position',[450 35 125 20].*WS,...
        'String','Add Blobs','Callback','nirs_spm_image(''addblobs'');','ToolTipString','superimpose activations');
    st.fg = fg;
end;
