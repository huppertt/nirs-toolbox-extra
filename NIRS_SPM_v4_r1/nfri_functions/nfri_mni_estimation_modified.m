function  nfri_mni_estimation(Origin, Others)
...
OtherC      = ones(size(OtherH));   % !!!!! % ˆÓ–¡•s–¾‚ÈƒR?ƒ“ƒg
% OtherCMean  = ones(PointN,3);       % OtherCMean ‚ÍŽQ??³‚??¹‚?B
...
%%% for NIRS-SPM
global NFRI_result
NFRI_result = affineEstData;



%% add the code to save the result as text file. (2009. 10. 8);
% WriteTXT(loadedData, affineEstData);
%
% function WriteTXT(loadedData, affineEstData )
% global fullPathList;
% fullPathRep = fullPathList.Rep;
% text_fname = [fullPathRep(1:end-3) 'txt'];
% fid = fopen(text_fname, 'w');
% for kk = 1:size(affineEstData.OtherC,1)
%     fwrite(fid, num2str(affineEstData.OtherC(kk,:)),'char');
% end
% fclose(fid);
...
while 1
    ...
    lXYZ = sscanf(tline, '%f');
    if isnumeric(lXYZ') & isempty(lXYZ') == 0
        XYZ = [XYZ; lXYZ'];
    else
        XYZ = [XYZ; NaN NaN NaN];
    end
