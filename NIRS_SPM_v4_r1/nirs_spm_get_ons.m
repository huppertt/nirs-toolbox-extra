function [U] = nirs_spm_get_ons(SPM,s,flag)
...
%      - 1: specify the model for design matrix
%      - 1: for the input of one trial 
%
...
%     elseif flag == 1
    ...
    elseif flag == 2
        ...
        mark_value = unique(vector_onset(index));
        v = length(mark_value);        
    elseif flag == 3 
        v = 1;        
        spm_input('number of conditions/trials: 1', '+1','d');        
    end
...
for i = 1:v
    ...
    if ~length(ons)
        ...
        elseif flag == 2
            ...
            index = find(vector_onset == mark_value(i));
            step(index+1) = 1;
            diff_step = diff(step);
    ...
    if ~length(dur)
        ...
        elseif flag == 2
            ...
            dur = index2- index;
        elseif flag == 3
            str = 'stimulus duration[s] (events = 0)';
            while 1
                dur = spm_input(str,'+1','r',' ',[Inf 1]);
                if length(dur) == 1
                    dur    = dur*ones(size(ons));
                end
                if length(dur) == length(ons), break, end
