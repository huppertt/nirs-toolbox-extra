...
axis(handles.axes1);
handles.hi=[];
handles.hi=plotmesh4probe(nodes,faces,'facecolor',skin,'edgecolor','none','FaceAlpha',1);
...
delete(toolsh{1,1}([1:6 9 15 16]))

hold on

handles.hg=[];
handles.hg=plotmesh4probe(GM_nodes,GM_faces,'facecolor',cortex,'edgecolor','none','FaceAlpha',0);
set(handles.hg,'Tag','gm');
view(90,0)
zoom(0.8);
...
if size('handles.refpts',2)>1
    ...
    for i=1:size(handles.refpts,1)
        ...
        %%%% change orientation
        if strcmp(handles.refpts{i,3},'LIA')
           XYZnew=[3 -1 2];
        end
        L=find(XYZnew<0);
        if ~isempty(L)
