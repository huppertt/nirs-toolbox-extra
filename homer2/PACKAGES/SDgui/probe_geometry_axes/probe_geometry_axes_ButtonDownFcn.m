function probe_geometry_axes_ButtonDownFcn(hObject, eventdata, handles)
    ...
    if size(p,1)>1
        ...
        if length(isrc)==1 & length(idet)==1
            q = menu('What action do you want to take with the selcted optodes?','Delete','Create/Delete Channel','Cancel');
            src = optpos_src(isrc,:);
            det = optpos_det(idet,:);
        elseif ~isempty(isrc) | ~isempty(idet)
            q = menu('What action do you want to take with the selcted optodes?','Delete','Cancel');
        end
        
        % Check if we need to delete optodes
        if q==1
            menu('This feature will be implemented in the next Homer2 release.','OK');
            src = [];
            det = [];
        end
