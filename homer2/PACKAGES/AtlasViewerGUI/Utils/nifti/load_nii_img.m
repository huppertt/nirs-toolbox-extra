...
% - Modified by Jay Dubb (jdubb@nmr.mgh.harvard.edu) on Dec. 5, 2008 to fix 
%   a problem encountered on Windows XP Matlab version 7.4.0.287. 
%   The function read_image was failing because fread was reading with 
%   the precision argument set to '*int32'. This didn't work in binary mode
%   the 3D NIFTI image files. Binary is the default mode fopen mode on windows. 
%   Changing the precision argument to 'int32' fixes things. 
   ...
   if filetype == 0 | hdr.dime.datatype == 1 | isequal(hdr.dime.dim(5:8),ones(1,4)) | ...
	(isempty(img_idx) & isempty(dim5_idx) & isempty(dim6_idx) & isempty(dim7_idx))
	 ...
	 img_siz = img_siz * 3;
      ...
      img = fread(fid, img_siz, precision);
         ...
         for i6=1:length(dim6_idx)
			...
			img_idx(t), dim5_idx(i5),dim6_idx(i6),dim7_idx(i7)) -1;
	             ...
	             img_siz = img_siz * 3;
                  ...
                  %  in img_siz times
                  %
                  img = [img fread(fid, img_siz, precision)];
               end
            end
         end
      end
