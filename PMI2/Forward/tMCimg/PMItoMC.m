% PMITOMC  Turn PMI structures into tMCimg config files
%
% PMItoMC(SD, Medium, basename, nPh, segfile);
%
...
function PMItoMC(SD, Medium, basefile, nPh, segfile)
...
fprintf(fid, 'dx    = %f\n', Medium.CompVol.XStep);
fprintf(fid, 'dy    = %f\n', Medium.CompVol.YStep);
fprintf(fid, 'dz    = %f\n', Medium.CompVol.ZStep);
fprintf(fid, 'nxvox = %d\n', nx);
fprintf(fid, 'nyvox = %d\n', ny);
fprintf(fid, 'nzvox = %d\n', nz);
...
fprintf(fid, '\ntissue { ');
fprintf(fid, '\tmua = %f ', Medium.Muao(iLmb) / 10);
fprintf(fid, '\tmus = %f ', Medium.Muspo(iLmb) / 10);
fprintf(fid, '\tg   = %f ', 0.0);
fprintf(fid, '\tn   = %f ', Medium.idxRefr(iLmb));
fprintf(fid, '}\n');
