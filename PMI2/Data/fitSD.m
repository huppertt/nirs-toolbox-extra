% fitSDAmp   Compute Source-Detector (complex) scaling vector
%
...
function[SrcAmp, DetAmp, DataAmp] = fitSD(SD, Medium, MeasList, ...
					     data, Phi0, Debug);
...
% mldivide has problems with sparse complex matrices

if (~isreal(Jamp) | ~isreal(Phi1))
   % Well-posed matrix, should converge rapidly
   sdfit(nzel) = full(Jamp(:,nzel)) \ Phi1;
else
...
% Clean up numerical slop from RF into CW components

if (~isreal(data))
   ifrq = find(SD.ModFreq == 0);

   if (~isempty(ifrq))
      SrcAmp(:,:,ifrq) = real(SrcAmp(:,:,ifrq));
      DetAmp(:,:,ifrq) = real(DetAmp(:,:,ifrq));
   end
end
